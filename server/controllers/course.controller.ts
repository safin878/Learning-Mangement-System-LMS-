...
// get single courses without purchasing
...
export const addReview = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userCourseList = req.user?.courses;
      const courseId = req.params.id;
      //check if courseId is already exist in userCourseList based on_id
      ...
      if (course) {
        course.ratings = avg / course.reviews.length; // one example we have 2 reviews one i 5 another one is 4 math=9/2 4.5
      }
      await course?.save();
      const notification = {
...
export const addReplyToReview = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!review.commentsReplies) {
        review.commentsReplies = [];
      }

      review.commentsReplies.push(replyData);
      await course?.save();
      res.status(200).json({
        success: true,
        course,
